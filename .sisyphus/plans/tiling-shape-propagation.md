...
- [ ] Implement `StreamShapeAdapter` IP core to handle dynamic reshaping between streams with varying bitwidths (e.g., $T=2$ to $T=4$).
- [ ] Update `src/stitcher.py` to perform topological traversal of the IP graph.
- [ ] Pass `StreamShape` objects during IP constructor calls.
- [ ] Automatically insert `StreamShapeAdapter` IP cores where `src.output_shape.T != dst.input_shape.T`.
- [ ] Add validation logic to ensure total tensor size (`N`) matches even if tiling factors (`T`) differ.

## Phase 4: Transformer Block Refactor
- [ ] Update `src/transformer_block.py` to accept top-level parameters (`seq_len`, `emb_dim`, `T`).
- [ ] Remove hardcoded parameters within the block.
- [ ] Ensure the block relies on the stitcher's shape propagation for internal connections.
...
## Verification
- [ ] Create unit tests to verify `StreamShape` propagation across different IP configurations.
- [ ] Add a test case for `StreamShapeAdapter`.
- [ ] Verify `TemporalGEMMCore` simulation output for multi-beat scenarios.